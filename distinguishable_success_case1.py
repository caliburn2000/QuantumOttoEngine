# -*- coding: utf-8 -*-
"""
Created on Mon May  1 10:36:41 2023
...
def eigvals_distinguishable(ntot,betac,betah,omegai,omegaf):
        ...
        for j in range(len(g0[0])):
            ...
            for n in range(N):
                for m in range(n, N):
                    ...
                    Hi[n,m] = (n1+n2+1)*(delta1*delta2)+((g0[i,j][0]/(np.sqrt(2*np.pi))*f_n1*f_n2*f_m1*f_m2))
                    Hf[n,m] = (n1+n2+1)*(delta1*delta2)+((g0[i,j][1]/(np.sqrt(2*np.pi))*f_n1*f_n2*f_m1*f_m2))
    ...
    # return{"energy_initial":ENERGY_i, "energy_final":ENERGY_f, "occ_i":OCC_i,"occ_f": OCC_f, "interaction_matrix":g0}
...
E_i, E_f, occ_i, occ_f, interaction_matrix = eigvals_distinguishable(ntot,betac,betah,omegai,omegaf)
...
fig, ax = plt.subplots()
scatter = ax.scatter(x_vals, y_vals, c=efficiency.ravel(), cmap='viridis', s=10)
...
x_vals = extract_vals(0, interaction_matrix)
fig, ax = plt.subplots()
scatter = ax.scatter(x_vals, y_vals, c=work.ravel(), cmap='viridis', s=100)
cbar = plt.colorbar(scatter)
plt.xlabel('$g_{f}$',fontsize =20)
plt.ylabel('$g_{i}$',fontsize =20)
